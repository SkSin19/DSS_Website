...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  const initialModel =
    typeof resolvedSearchParams.model === "string" ? resolvedSearchParams.model : "";
  ...
  const selectedSubCategories = toArray(
    resolvedSearchParams.subCategory || resolvedSearchParams.subCategories,
  );
  ...
  if (pagination?.totalPages && requestedPage > pagination.totalPages) {
    const fallbackResponse = await getProductsPageFromApi({
      ...queryParams,
      page: pagination.totalPages,
    }).catch(() => ({ products: [], pagination }));
    products = fallbackResponse.products;
    pagination = fallbackResponse.pagination;
  }

  const companies = Array.from(
    new Set(catalog.map((p) => p.company).filter(Boolean)),
  ).sort();
  const categories = Array.from(
    new Set(catalog.map((p) => p.category).filter(Boolean)),
  ).sort();
  const subCategories = Array.from(
    new Set(catalog.flatMap((p) => p.subCategories || []).filter(Boolean)),
  ).sort();
  ...
  const hasActiveFilters =
    initialName || initialModel || selectedCompanies.length ||
  ...
  return (
    <section className="bg-white select-none">
      ...
      <Container>
        <div className="flex gap-6 py-6 sm:py-8 lg:py-10">
          ...
          <div className="min-w-0 flex-1">
            ...
            {products.length > 0 ? (
                  ...
                  {products.map((product, index) => (
                    ...
                    >
                      ...
                      <div className="flex sm:hidden items-stretch min-h-24">
                        <div
                          className="relative w-28 shrink-0 overflow-hidden p-2 ring-1 ring-gray-200"
                      ...
                      <div className="hidden sm:block">
                        <div
                          className="relative aspect-4/3 overflow-hidden p-2 sm:p-2.5 ring-1 ring-gray-200"
                          style={{ backgroundColor: THEME_COLORS.shadowGrey100 }}
                        >
                ...
                {pagination?.totalPages && pagination.totalPages > 1 ? (
                  <div className="mt-7 flex flex-wrap justify-center gap-1.5 sm:gap-2">
                    {Array.from({ length: pagination.totalPages }, (_, index) => {
                      ...
                      const isActive =
                        pageNumber === (pagination.currentPage || requestedPage);
                      const query = new URLSearchParams();
                      if (initialName) query.set("name", initialName);
                      if (initialModel) query.set("model", initialModel);
                      selectedCompanies.forEach((c) => query.append("company", c));
                      selectedCategories.forEach((c) => query.append("category", c));
                      selectedSubCategories.forEach((c) => query.append("subCategory", c));
                      ...
                      return (
                        <Link
                          ...
                          className={`inline-flex h-7 w-7 items-center justify-center rounded-sm border text-[11px] font-semibold transition-colors sm:h-8 sm:w-8 sm:text-xs ${
                            isActive
                              ? "border-red-600 bg-red-600 text-white"
                              : "border-gray-300 bg-gray-100 text-gray-700 hover:border-red-300 hover:bg-gray-200"
                          }`}
                        >
                          {pageNumber}
                        </Link>
              ...
              <div
                className="rounded-4xl border border-gray-200 p-8 text-gray-600"
                style={{ backgroundColor: THEME_COLORS.shadowGrey50 }}
              >
                No products matched the current filters. Adjust the selections or
                clear them to browse the full catalog.
              </div>
            )}
          </div>
        </div>
